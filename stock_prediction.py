...
def prepare_data(past_n,start_date,end_date,keyword_num,term_offset):
    ...
    all_sentiments_file_name = "{0}_sentiments_{1}.npy".format(data_root_dir + "/" + stock_symbol,get_date_duration_string(start_date,end_date))
    ...
    for i in range(delta.days + 1):
        ...
        if(pc.has_date(current_date)):
            ...
            all_sentiments.append(get_polarity_for_date(current_date))
    ...
    np.save(all_sentiments_file_name,all_sentiments)
...
def classification(classifier,past_n,start_date,end_date,keyword_num,term_offset,cv):
    ...
    all_data_files_exist = file_exists(trends_data_file_name) and file_exists(vectorized_data_file_name) and file_exists(past_n_trends_matrix_file_name) and file_exists(all_sentiments_file_name)
    ...
    all_sentiments = np.load(all_sentiments_file_name)
    all_sentiments = all_sentiments.reshape(-1, 1)
    ...
    data_train_matrix = np.hstack((data_train_matrix,all_sentiments))
...
def plot_for_classifier_results_for_sentiment():
    ...
    plt.ylabel("AVG Accuracy")
    plt.title("Prediction Accuracy With Sentiment Analysis")
    ...
    for classifier in classifiers:
        ...
        for result in result_array:
            params = result["params"]
            my_map[params["term_offset"]] = result["avg_score"]
    ...
    plt.savefig("sentiment" + "_accuracy_trend_plot")
    plt.close()
    ...
    for classifier in classifiers:
        ...
        for result in result_array:
            if(count<n):
                ...
                print("{0},{1},{2}".format(params["term_offset"],result["avg_score"],result["predicted_metrics"]))
...
def classification_with_sentiments(classifier,start_date,end_date,term_offset,cv):
    all_sentiments_file_name = "{0}_sentiments_{1}.npy".format(data_root_dir + "/" + stock_symbol,get_date_duration_string(start_date,end_date))
    trends_data_file_name = '{0}_all_trends_{1}_offset{2}.npy'.format(data_root_dir + "/" + stock_symbol,get_date_duration_string(start_date,end_date),term_offset) 

    all_data_files_exist = file_exists(trends_data_file_name) and file_exists(all_sentiments_file_name)
    if(not all_data_files_exist):
        prepare_data(1,start_date,end_date,100,term_offset)

    all_sentiments = np.load(all_sentiments_file_name)
    all_trends = np.load(trends_data_file_name)
    all_sentiments = all_sentiments.reshape(-1, 1)
    scores = cross_val_score(classifier, all_sentiments, all_trends, cv=cv,scoring='average_precision')
    return all_sentiments, all_trends, scores


def prepare_data_for_prediction_with_sentiments(start_date,end_date,term_offset):
    # print("[data prep] preparing training data...")
    trends_data_file_name = '{0}_all_trends_{1}_offset{2}.npy'.format(data_root_dir + "/" + stock_symbol,get_date_duration_string(start_date,end_date),term_offset) 
    all_sentiments_file_name = "{0}_sentiments_{1}.npy".format(data_root_dir + "/" + stock_symbol,get_date_duration_string(start_date,end_date))
...
def predict_with_classifier_and_sentiments(classifier,start_date,end_date,term_offset):
    # print("[prediction] loading training data from .npy files...")
    trends_data_file_name = '{0}_all_trends_{1}_offset{2}.npy'.format(data_root_dir + "/" + stock_symbol,get_date_duration_string(start_date,end_date),term_offset) 
    all_sentiments_file_name = "{0}_sentiments_{1}.npy".format(data_root_dir + "/" + stock_symbol,get_date_duration_string(start_date,end_date))

    prepare_data_for_prediction_with_sentiments(start_date,end_date,term_offset)

    all_trends = np.load(trends_data_file_name)
    all_sentiments = np.load(all_sentiments_file_name)
    all_sentiments = all_sentiments.reshape(-1, 1)
    data_train_matrix = all_sentiments
...
def get_sentiment_matrix_for_period(start_date,end_date):
    sentiments = []
    delta = end_date - start_date
    for i in range(delta.days + 1):
        current_date = start_date + timedelta(days=i)
        sentiments.append(get_polarity_for_date(current_date))
    return sentiments


def get_polarity_for_date(date):
    polarities = []
    polarities.append(0)
    for news in nc.get_news_for_date(date):
        news_sentiment = TextBlob(news).sentiment
        polarities.append(news_sentiment.polarity)
...
def train_with_params_and_sentiments(classifier,start_date,end_date,cv,term_offset_range):
    all_results = []
    for term_offset in term_offset_range:
        params = {}
        params['term_offset'] = term_offset
        training_data, training_labels, scores = classification_with_sentiments(classifier["classifier"],start_date,end_date,term_offset,cv)

        prediction_start_date = date(2017,4,2)
        prediction_end_date = date(2017,6,2)
        classifier["classifier"].fit(training_data,training_labels)
        predicted_metrics = predict_with_classifier_and_sentiments(classifier,prediction_start_date,prediction_end_date,term_offset)
        ...
        result['predicted_metrics'] = predicted_metrics
        all_results.append(result)
        print_and_write_to_file("** term_offset={0} | AVG_SCORE = {1}"
            .format(result['params']['term_offset'],np.mean(result['scores'])))
        print_and_write_to_file("** scores: [{0}]".format(', '.join([str(x) for x in result['scores']])))
        print_and_write_to_file("-------------------------------------------------------------------------")
...
def new_main():
    ...
    for classifier in classifiers:
        all_results = train_with_params_and_sentiments(classifier,start_date,end_date,cv,term_offset_range)
        classifier_to_all_results_map[classifier["name"]] = all_results
        sorted_results = sorted(all_results, key=lambda k: k['avg_score'], reverse=True) 
        classifier_to_all_sorted_results_map[classifier["name"]] = sorted_results
        count = 0
...
# main()
# new_main()
plot_for_classifier_results_for_sentiment()
get_top_n_results(10)
